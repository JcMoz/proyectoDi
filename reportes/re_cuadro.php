...
while ($ver_act = $sacar_acti->fetch_assoc()) {
    $x = $pdf->GetX();
    $pdf->myCell(10, $h, $x, utf8_decode('N°'));
    $x = $pdf->GetX();
...
while ($row_notas = $notas->fetch_assoc()) {
    $pdf->Cell(10, 6, utf8_decode($row_notas['rownum']), 1, 0, 'C');
    $pdf->Cell(59, 6, utf8_decode($row_notas['ape_alumno'] . ' ' . $row_notas['nom_alumno']), 1, 0, 'L');
    $pdf->Cell(26, 6, $row_notas['nota1'], 1, 0, 'C');
    $pdf->Cell(26, 6, $row_notas['nota2'], 1, 0, 'C');
    $pdf->Cell(26, 6, $row_notas['nota3'], 1, 0, 'C');
    $pdf->Cell(10, 6, $row_notas['por35'], 1, 0, 'C');
    $pdf->Cell(26, 6, $row_notas['nota4'], 1, 0, 'C');
    $pdf->Cell(26, 6,0, 1, 0, 'C');
    $pdf->Cell(26, 6, 0, 1, 0, 'C');
    $pdf->Cell(10, 6,0, 1, 0, 'C');
    $pdf->Cell(26, 6, 0, 1, 0, 'C');
    $pdf->Cell(26, 6,0 ,1, 0, 'C');
    $pdf->Cell(26, 6, 0, 1, 0, 'C');
    $pdf->Cell(10, 6,0, 1, 0, 'C');
    $pdf->Cell(10, 6, 0, 1, 0, 'C');
    $pdf->Cell(10, 6, utf8_decode($row_notas['notaF']), 1, 1, 'C');
}
