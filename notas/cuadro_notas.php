                ...
                <div class="container">
                    <form action="" id="FORMULARIO_VALIDADO"  method="post" class="form-register" >
                        ...
                        <div class="panel-body">
                            ...
                            $paMostrar = mysqli_query($conexion, "SELECT *FROM actividades_materia a, asignacion_m m WHERE a.id_asignacion_m=m.id_asig_m AND a.id_materia='$materia'"
                            ...
                            if (mysqli_num_rows($paMostrar) > 0) {
                                ...
                                $extraer_id = mysqli_query($conexion, "SELECT *FROM actividades_materia a, asignacion_m m WHERE a.id_asignacion_m=m.id_asig_m AND a.id_materia='$materia'"
                                        . " AND m.id_docente='$profesor' AND a.id_grado='$grado' AND a.estado_a='enProceso'");
                                while ($xyz = mysqli_fetch_array($extraer_id)) {
                                    $a_actividad = $xyz['id_asignacion_m'];
                                    $idActividad = $xyz['id_actividades_m'];
                                    $pe = $xyz['periodo'];
                                    $a1 = $xyz['act_1'];
                                    $a2 = $xyz['act_2'];
                                    $a3 = $xyz['act_3'];
        ...
        if (isset($_POST['procesar'])) {
            try {
                ...
                function () {
                    ...
                    //no entra aqui y no necesito que entre solo para que
                    //se detenga el proceso mediante este else XD
                }
             } 
               else {
             foreach ($_POST['id_alu'] as $ids) {
                ...
                $comprobar_cambio = mysqli_query($conexion, "SELECT*FROM actividades_materia WHERE id_asignacion_m='$a_actividad' AND id_materia='$materia' AND id_grado='$grado' AND estado_a='enProceso'");
                if (mysqli_num_rows($comprobar_cambio) > 0) {
                    //si esta en proceso
                    mysqli_query($conexion, "UPDATE actividades_materia SET estado_a='$aux' WHERE id_actividades_m='$idActividad'");
                    if ($pe == 1) {
                        $cambio1 = mysqli_query($conexion, "SELECT *FROM actividades_materia a, asignacion_m m WHERE a.id_asignacion_m=m.id_asig_m AND a.id_materia='$materia'"
                                . " AND m.id_docente='$profesor' AND a.id_grado='$grado' AND a.estado_a='esperando' AND periodo=2");
                        while ($esperando1 = mysqli_fetch_array($cambio1)) {
                            $x = $esperando1['id_actividades_m'];
                        }
                        mysqli_query($conexion, "UPDATE actividades_materia SET estado_a='$aux1' WHERE id_actividades_m='$x'");
                    }
                    if ($pe == 2) {
                        $cambio1 = mysqli_query($conexion, "SELECT *FROM actividades_materia a, asignacion_m m WHERE a.id_asignacion_m=m.id_asig_m AND a.id_materia='$materia'"
                                . " AND m.id_docente='$profesor' AND a.id_grado='$grado' AND a.estado_a='esperando' AND periodo=3");
                        while ($esperando1 = mysqli_fetch_array($cambio1)) {
                            $x = $esperando1['id_actividades_m'];
                        }
                        mysqli_query($conexion, "UPDATE actividades_materia SET estado_a='$aux1' WHERE id_actividades_m='$x'");
                ...
                function () {
                    ...
                    //no entra aqui y si entrara no necesito que aga algo
                    //solo que se detenga el proceso mediante este else XD
                }
